#!/usr/bin/env python3
"""Lazada near-SLA alert - standalone data fetcher.

Fetches orders approaching their ship-by SLA deadline, builds a simple
Telegram-compatible alert, and prints a JSON envelope to stdout.

Usage:
    python3 scripts/lazada_near_sla_alert.py --channel -1004450247696
    python3 scripts/lazada_near_sla_alert.py --channel -1004450247696 --hours 6
...
def build_payload(
    *,
    ...
    orders_with_sla = int(sla_data.get("orders_with_sla", 0))
    ...
    blocks = [
        {
            ...
            "text": {
                "type": "mrkdwn",
                "text": (
                    f":lazada: *Lazada Near-SLA Alert* ({now.strftime('%a %b %-d, %Y')})\n\n"
                    f"Orders with items approaching SLA deadline (within *{hours_threshold}h*): *{near_sla_count}*\n"
                    f"Affected orders: *{orders_with_sla}*\n\n"
                    f"Review and ship before the deadline."
                ),
            },
        },
    ]

    text = f"Lazada Near-SLA Alert ({now.strftime('%Y-%m-%d %H:%M')})"
    return {
        "ok": True,
        "platform": "lazada",
        "channel": channel,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Lazada near-SLA alert - fetch near-deadline orders and print JSON payload"
    )
