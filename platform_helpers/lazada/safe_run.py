...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Safe Lazada helper runner with optional save path"
    )
    parser.add_argument(
        "--save-json",
        dest="save_json",
    ...
    parser.add_argument(
        ...
        help="Lazada helper command arguments after --",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
            ...
            json.dumps(
                {
                    ...
                    "error": "Missing command arguments. Example: python3 -m lazada_helper.safe_run -- orders get --days 7",
                },
                ensure_ascii=True,
            )
            + "\n"
    ...
    try:
        ...
        payload = {
            "ok": False,
            "status": "wrapper_error",
            "error": "Failed to parse lazada_helper.cli output as JSON",
            "raw_output": output,
        }
        exit_code = 1
