...
type Platform = "slack" | "discord" | "lark";
...
function resolveChannelLocator(input: string): ResolvedChannel {
  const trimmed = input.trim();
  if (!trimmed) {
    throw new Error("channelId is required");
  }

  // Accept "workspaceId::channelId" pairs for ambiguity resolution (same
  // convention used by ode-task / ode-cron CLIs).
...
export function registerSendRoutes(app: Elysia): void {
  /**
   * Unified file upload endpoint powering `ode send file`. Replaces the
   * per-platform `upload_file` branch of `/api/action` — callers no longer
   * need to know which messaging provider is behind the channel; the server
