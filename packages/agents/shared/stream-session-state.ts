...
export type InspectorToolState = SessionTool & {
  inputBuffer?: string;
};

export type StreamStateMaps = {
  textByIndex: Map<number, string>;
  thinkingByIndex: Map<number, string>;
  toolByIndex: Map<number, InspectorToolState>;
  toolById: Map<string, InspectorToolState>;
};
...
export function extractStreamRecord(
  type: string,
  eventData: Record<string, unknown>,
  eventProps: Record<string, unknown>,
  prefix: string
): StreamRawRecord | null {
  if (!type.startsWith(prefix)) return null;
  const candidate = eventProps.record ?? eventData.record;
  if (!candidate || typeof candidate !== "object") return null;
  return candidate as StreamRawRecord;
}

export function applyStreamRecordToState(
  state: SessionMessageState,
  record: StreamRawRecord,
  streamState: StreamStateMaps,
  options: { toolIdPrefix: string; agentLabel: string }
): void {
  ...
  if (record.type === "user") {
    ...
    for (const block of blocks) {
      ...
      const updated: InspectorToolState = {
        ...existing,
  ...
  if (record.type === "result") {
    state.phaseStatus = record.is_error ? `${options.agentLabel} reported an error` : "Finalizing response";
    return;
  }
